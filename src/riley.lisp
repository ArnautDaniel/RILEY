...
(defclass multi-pic ()
  ((item :col-type item-db
   ...
   (set :initarg :set
        :accessor multi-set
        :col-type :text)
   (check-in :initarg :check-in
             :accessor multi-check-in
             :col-type :text))
  (:metaclass mito:dao-table-class))
...
(defun multi-checked? (lam)
  (cond ((string= lam "yes") "yes")
        ((string= lam "no") nil)))
...
;;;Now supports png and jpg, need to add jpeg jpe and JPG extentions
  ...
  (let* ((current-invoice (find-invoice-from-cookie (cookie-in "current-invoice")))
    ...
    (let* ((new-img-name (concatenate 'string (db-set-name current-invoice)
      ...
      (let* ((new-img-path (make-pathname :directory (concatenate 'string invoice-location "webimg/")
             ...
             (file-ext (pathname-type new-img-name))
             (img ((lambda () (cond ((string= file-ext "jpg")
                                     (read-jpeg-file new-img-path))
                                    ((string= file-ext "png")
                                     (read-png-file new-img-path))
                                    (t 1))))))
...
(define-easy-handler (multi-pic-check-in :uri "/multi-pic-check-in")(id)
  (let ((multi-pic-list (loop for post-parameter in (hunchentoot:post-parameters*)
                           if (equal (car post-parameter) "checkbox[]")
                           collect post-parameter)))
